...
An `<InstancedMesh>` is used in conjunction with the [`<Instance>`](#instance) component nested in a slot with the name _"instances"_:
...
An `<Instance>` component outside the slot "instances" creates an infinite loop.
:::
