...
inside child components. The implementation roughly follows a subset of the concept
established by the [React Suspense
API](https://react.dev/reference/react/Suspense) and has certain limitations.

The basic idea is to allow a component to make decisions based on the loading
state of a child component. This is especially useful when loading a model in a
child component while the parent component is already rendered. The parent
component can then decide to show a loading indicator or a fallback component
while the child component is loading.
...
## UX considerations and implementation limitations
...
- Directly citing the [React Suspense API](https://react.dev/reference/react/Suspense#revealing-nested-content-as-it-loads):
...
- The `<Suspense>` component will **mount** child components in the default slot immediately and hide the contents
  as long as they are suspended. This means that any side effects (for instance `useFrame` hooks) that are triggered
  by the child components will be executed immediately.
- A `<Suspense>` component can be re-suspended if a child component invokes `suspend` again. The property `final` on the component `<Suspense>` can be used to prevent this behavior.

## Inner workings

The `<Suspense>` component will only ever show one of the available slots at any time:
...
It's important to keep in mind that while the contents of the default slot are not visible, they are still mounted and
the component is otherwise fully functional. This means that any side effects (for instance `useFrame` hooks) that are triggered
by the child components will be executed immediately. As long as a component is suspended, it is added to a `THREE.Group` which
