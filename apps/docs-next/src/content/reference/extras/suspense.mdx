...
<Tip type="warning">
  ...
  effects (for instance `useFrame` hooks) that are triggered by the child components will be
  executed immediately. As long as a component is suspended, it is added to a `THREE.Group` which is
