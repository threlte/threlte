...
The component `<Merged>` takes existing `THREE.Mesh` instances and creates a `THREE.InstancedMesh` per `THREE.Mesh`. This is especially useful if you want to instanciate a lot of meshes that have been loaded with hooks like [`useGltf`](/docs/reference/extras/use-gltf).
...
Load a gltf file with the [`useGltf`](/docs/reference/extras/use-gltf) hook and pass the result to the `<Merged>` component. The [slot prop](https://svelte.dev/docs#template-syntax-slot) `components` can be used to instanciate a mesh multiple times.

```svelte
<script lang="ts">
  import { useGltf, Merged } from '@threlte/extras'
  ...
  // passing that map to the `<Merged>` component, it will
  // automatically filter out all nodes that are no
...
{#if gltf}
  ...
  -->
  <Merged meshes={$gltf.nodes} let:components={{ Cube }}>
...
<Tip type="tip">
  When using `<Merged>` with a large gltf file, be aware that `<Merged>` will
...
If you don't want to use the `useGltf` hook, you can also pass an array of `THREE.Mesh` instances to the `<Merged>` component.

```svelte
<script lang="ts">
  import { Merged } from '@threlte/extras'
  import { Mesh, BoxGeometry, MeshStandardMaterial } from 'three'

  const meshes = [
    new Mesh(new BoxGeometry(), new MeshStandardMaterial()), // MeshA
    new Mesh(new BoxGeometry(), new MeshStandardMaterial()), // MeshB
    new Mesh(new BoxGeometry(), new MeshStandardMaterial())  // MeshC
  ]
</script>
...
<!--
    ...
    <MeshB> and <MeshC>.
  -->
<Merged meshes={meshes} let:components={[MeshA, MeshB, MeshC]}>
  <Mesh position.y={2} position.x={-1}>
</Merged>
```
