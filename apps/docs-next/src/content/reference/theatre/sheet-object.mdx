...
The component `<SheetObject>` represents a Theatre.js sheet object. This component is creating a Theatre.js sheet object under the
hood and therefore all original concepts apply.

A Theatre.js [Sheet Object](https://www.theatrejs.com/docs/latest/manual/objects) pools the properties of a **logical entity** which
can be a anything from a single material, multiple meshes or a complete scene. The component `<SheetObject>` is able to
pool the properties of a logical entity and make them editable and animatable in the Theatre.js studio.

<Example
  path="theatre/sheet-object"
  iframe
/>

## Usage

You have the following options to add properties to the underlying sheet object:

- Provide the `props` property to the component `<SheetObject>` for a manual property declaration.
...
- Use the component `<Sync>` from the `<SheetObject>` slot props to automatically pick up properties from its parent.
...
- Use the component `<Declare>` to co-locate an object and its properties with manual property declaration.
...
# THIS MARKS THE END OF HAND WRITTEN DOCS FOR `<SheetObject>`, THE REST IS COPIED FROM THE OLD DOCS!!!

## Manual Property Declaration

To manually declare properties, provide the `props` property to the component `<SheetObject>`.
...
<SheetObject
  ...
  let:values
>
  <T.Mesh position={[values.x, values.y, values.z]} />
</SheetObject>
```
...
Similarly to the pierced props of [`<T>`](/docs/reference/core/t), the props of the component `<Editable>` can be notated in the same fashion:
