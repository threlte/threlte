...
- Use the component `<AutoProps>` from the `<SheetObject>` slot props to automatically pick up properties from its parent.

```svelte
<SheetObject let:AutoProps>
  <T.MeshBasicMaterial>
    <AutoProps
      color
      roughness
...
- Use the component `<ManualProps>` to co-locate an object and its properties with manual property declaration.

```svelte
<SheetObject let:ManualProps>
  <ManualProps
