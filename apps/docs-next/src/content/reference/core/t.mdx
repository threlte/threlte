...
<Tip type="tip">
  All native elements ending with "Material" (such as `MeshStandardMaterial`) receive
  `attach="material"`, and all elements ending with "Geometry" receive
...
Adding an event listener to a component will also add the corresponding event listener to the three.js class instance. The event will be forwarded and the native payload available as the first argument to the event listener.
...
<T
  is={OrbitControls}
  on:change={(e) => {
    console.log('OrbitControls changed. payload:', e)
  }}
/>
```
