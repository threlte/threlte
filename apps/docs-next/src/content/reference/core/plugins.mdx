...
functionality. This means you can not only add arbitrary props and events to your `<T>` components but also
override the default behavior of the `<T>` component.

Plugins can add arbitrary code to every `<T>` component instance and can act as if
they were part of the `<T>` component itself. You have full access to props, event listeners and the
component instance itself. For an advanced example of what a plugin can do, see the
[interactivity plugin of `@threlte/extras`](/docs/reference/extras/interactivity).
