...
<Tip type="tip">
  `CurrentWritable` is a custom Threlte store. It's a regular writable store that also has a
  `current` property which is the current value of the store. It's useful for accessing the value of
  a store in a non-reactive context, such as in loops.
</Tip>

## Authoring Reusable Components

Svelte has no native way of forwarding arbitrary event listeners from inside a component. To make it easier
to author components the `forwardEvents` function from `'@threlte/core'` can be used to forward event listeners
defined on the component itself to an instance of `<T>`.

To make things clearer, let's start with a simple
component that _doesn't_ forward events:
...
This component renders a box that changes color when the pointer is over it. The `useCursor` hook
provides visual feedback when the pointer is over the box. Because we pass the `$$restProps` to `<T.Mesh>`
we can use all props that are available on `<T.Mesh>` from the parent component. We can now implement this
component in our Scene like this:
...
This works fine, but we can't use any events like `on:click` on the `<Box>` component. We can forward all event listeners defined on `<Box>` by using the `forwardEvents` function. Let's update our `<Box>` component to forward all event handlers defined on `<Box>` to the component `<T.Mesh>` inside `Box.svelte`:

```svelte title="Box.svelte" {2}m {6,22}+
<script lang="ts">
...
Now we can use the `on:click` event handler on the `<Box>` component:

```svelte title="Scene.svelte" {10-12}+
<script>
  import { interactivity } from '@threlte/extras'
  import Box from './Box.svelte'
