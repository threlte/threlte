...
Svelte has no native way of forwarding arbitrary event handlers from inside a component. To make it easier
to author components the `forwardEventHandlers` function from `'@threlte/core'` can be used to forward event handlers
defined on the component itself to an instance of `<T>`.

To make things clearer, let's start with a simple
component that _doesn't_ forward event handlers:
...
This works fine, but we can't use any events like `on:click` on the `<Box>` component. We can forward all
event handlers defined on `<Box>` by using the `forwardEventHandlers` function. Let's update our `<Box>`
component to forward all event handlers defined on `<Box>` to the component `<T.Mesh>` inside `Box.svelte`:
...
Now we can use the `on:click` event on the `<Box>` component:
