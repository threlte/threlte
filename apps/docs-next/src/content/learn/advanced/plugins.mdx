...
Plugin are a way to extend the functionality of the `<T>` component. They are a way to inject code into the component without having to fork the codebase.

### What it looks like

There are two ways to inject a plugin into the `<T>` component. The first one is to use the `injectPlugin` function. This function is available in the `@threlte/core` package.
...
The second way is to provide `plugins` prop to the `<Canvas>` component. This is useful if you want to make a simple application that doesn't need to split up the code into multiple files.

```svelte
<Canvas plugins={[() => injectPlugin(({ ref, props }) => {
	console.log(ref, props)
})]}>
	<T.Mesh />
</Canvas>
