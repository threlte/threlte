...
Therefore we want to utilise 2D noise, and this example uses `createNoise2D` from npm package 'simplex-noise'.
...
(remember that planes are on their side by default, i.e. they extend in the x and y directions, and every z is 0)
...
6. This example intentionally used only part of the noise map (the middle 25%), in order to generate gentler terrain.
