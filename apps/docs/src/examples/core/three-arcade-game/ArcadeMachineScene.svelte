<script lang="ts">
	import { InteractiveObject, T, useFrame, useThrelte } from '@threlte/core'
...
<T is={Scene} bind:ref={$arcadeMachineScene} background={new Color(0x020203)}>
	...
	{#if nodes && materials}
		...
		<T is={Group} rotation.y={DEG2RAD * 180}>
			<T
				is={Mesh}
	...
	<T is={AmbientLight} intensity={8} color={$averageScreenColor} />
	<T is={AmbientLight} intensity={$whiteAmbientLightIntensity} color={'white'} />
	...
	<!-- Floor -->
	<T is={Mesh}>
		<T is={CylinderGeometry} args={[1, 1, 0.04, 64]} />
		<T is={MeshStandardMaterial} color={'#0f0f0f'} />
	</T>
</T>
