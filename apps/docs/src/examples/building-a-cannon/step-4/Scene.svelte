...
<!-- The shot is a dense ball: at this radius, density 30 makes it a little
     heavier than a brick, which matters once it starts hitting them. -->
{#if fireSignal > 0}
  {#key fireSignal}
    <T.Group position={muzzle}>
      <RigidBody bind:rigidBody={ball}>
        <Collider
          shape="ball"
          args={[BALL_RADIUS]}
          density={30}
