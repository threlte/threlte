...
Threlte's way of composing functionality with _trait components_ works very well in harmony with the new components `<T>` and `<Three>`. Be aware that these new component are supposed to be as thin as possible and do not provide event handling or viewport awareness by default. It can easily composed in though:
