[ShadowMesh](https://github.com/mrdoob/three.js/blob/master/examples/jsm/objects/ShadowMesh.js) is an addon that is used as very performant alternative to shadow mapping. It uses the renderer's stencil buffer and a locally computed matrix.
...
This is because the geometry of the shadow mesh is pulled from the mesh passed into its constructor at instantiatiation and does not update if the reference mesh's geometry is updated.
...
The **w** component of the light position vector controls the _spread_ of the shadow. Values closer to 0 are suited for directional light whereas values closer to 1 are for point lights.

## Offsetting and Orienting the Plane

The plane should be offset from the ground or floor by a small amount. This is done to avoid z-fighting. An easy way to do this is to create the plane first and then orient the mesh to look at the plane's normal
...
Unfortunately, there's not an easy way to get the normal of a plane geometry. It is possible but it involves accessing the geometry's buffer attributes.
