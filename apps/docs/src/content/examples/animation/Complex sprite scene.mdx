...
`SpriteDudes.svelte` adds sprites with random walk to the scene. One of them is controlled by the player with the use of `WASD` keys.
This example uses an untyped `useInstancedSprite()` hook within the `UpdaterWalking.svelte` component to update the sprites.

### One file, many animations (flyers)

`SpriteFlyers.svelte` is the sprite from the first section where we went over step by step how to work with the component.
...
`SpriteFlyersTyped.svelte` addresses this.

### Multiple files, one animation per (goblins)

`SpriteGoblin.svelte` buildis a spritesheet from multiple files, each of them containing a single animation. Similar to the
...
### Static sprites (trees)

The example in `SpriteAtlasTrees.svelte` demonstrates the setup of static sprites, as outlined in [Static sprites & Atlassing](/docs/reference/extras/instanced-sprite#static-sprites--atlassing).
