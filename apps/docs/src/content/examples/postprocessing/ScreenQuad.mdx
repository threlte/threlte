...
The basic idea of postprocessing is to draw the scene to a frame-buffer or texture that can then be sent into another shader. The output of this shader is used as the texture or material for a mesh that covers the screen. To do this we need two things:
...
const sizeAsState = fromStore(size)

$effect(() => {
  uSize.value.set(sizeAsState.current.width, sizeAsState.current.height)
})
