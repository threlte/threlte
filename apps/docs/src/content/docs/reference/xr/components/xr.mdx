...
The `<XR>` component prepares your scene for a WebXR session. It sets up context that is provided by the [`useXR`](/reference/xr/use-xr) hook.
...
<Tip type="warning">
  The `<XR>` component will set the [`<Canvas>`](/reference/core/canvas) property `renderMode="always"` when the user enters an XR session, due to being incompatible with `on-demand` or `manual`. It will set the original value once the session has ended.
</Tip>
