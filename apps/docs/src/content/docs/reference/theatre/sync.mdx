...
Use the component `<Sync>` from the [`<SheetObject>`](/reference/theatre/sheet-object) slot prop to automatically pick up properties from its parent.
...
Similarly to the pierced props of [`<T>`](/reference/core/t), the props of the component `<Sync>` can be anotated in the same fashion.
