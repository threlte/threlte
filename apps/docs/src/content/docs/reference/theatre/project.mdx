...
Theatre.js work is organized into projects that group animation [\<Sheet\>](/reference/theatre/sheet)s.

Projects also provide the means to inject configuration state exported as a JSON file from the [\<Studio\>](/reference/theatre/studio) back into your code through a prop: `<Project config={{ state }}>`.
...
The state of a project edited in the [\<Studio\>](/reference/theatre/studio) is saved in your browser's local storage, and can be [exported from within the studio interface](https://www.theatrejs.com/docs/latest/manual/projects#state). It's a JSON file containing all animated and static properties of all sheets of the project.
