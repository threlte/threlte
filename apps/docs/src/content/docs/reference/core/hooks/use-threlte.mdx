...
:::note
This hook relies on context passed down by the [`<Canvas>`](/reference/core/components/canvas) component and can only be used in a child of that component.
:::
...
If the [renderMode is set to `'on-demand'`](/reference/core/components/canvas) and you are manually editing objects or materials, be sure to invalidate the current frame to request a rerender:
...
If the [renderMode is set to `'manual'`](/reference/core/components/canvas) you must manually trigger a re-render:
