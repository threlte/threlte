...
Using [**attachments**](/reference/core/components/t#attach), the geometry as well as the material are assigned to the mesh. What if we want to change the color of the material to `"red"`? We can do this by using the `color` prop:
...
The event handler is called with an object containing a reference to the object.
You may return a callback that will run after the object is destroyed when the component unmounts or the `args` change.
