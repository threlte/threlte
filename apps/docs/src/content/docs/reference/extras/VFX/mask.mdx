...
:::caution
The Mask component requires Three.js to render with a stencil buffer. As of [r163](https://github.com/mrdoob/three.js/releases/tag/r163), stencil is set to `false` by default. To enable the stencil buffer, set it in your canvas's renderer: `<Canvas createRenderer={(canvas)=>{return new WebGLRenderer({canvas,stencil: true })}}>`. In prior versions the default is `true` already.
:::
