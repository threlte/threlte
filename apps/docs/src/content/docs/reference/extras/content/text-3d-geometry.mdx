...
[`<Suspense>` boundary](/reference/extras/suspense) will suspend
...
You can center a text by using the [`<Align>` component](/reference/extras/align) and calling the align slot prop when the text geometry is created.
