...
A convenience hook to use gltf animations loaded by a [\<GLTF>](/reference/extras/gltf) component or by the [`useGltf`](/reference/extras/use-gltf) hook.
...
For cases where you want to reuse parts of the GLTF such as materials, nodes, or the embedded camera, `useGltfAnimations` accepts a writable store as its first argument. [`useGltf`](/reference/extras/use-gltf) returns a store that can be directly passed to `useGltfAnimations`.
