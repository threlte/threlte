...
A [plugin](/reference/core/helpers/plugins) to add click, pointer and wheel events to your Threlte app.
...
<Tip type="tip">
  [`CurrentWritable`](/reference/core/utilities#currentwritable) is a custom Threlte store. It's a
  regular writable store that also has a `current` property which is the current value of the store.
  It's useful for accessing the value of a store in a non-reactive context, such as in loops.
</Tip>
