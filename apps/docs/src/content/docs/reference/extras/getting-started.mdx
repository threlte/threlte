...
The most notable one is probably the plugin [`interactivity`](/reference/extras/interactivity) that provides a way to interact with the scene. We can
...
and more. Let's look at an example where we use the hook [`useGltf`](/reference/extras/use-gltf)
to load an asset:
...
[`useLoader`](/reference/core/use-loader) internally.
