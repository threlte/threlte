...
child [`<Box>`](/reference/flex/box) components to resolve their layout.
...
method [`createClassParser`](/reference/flex/create-class-parser) or by
using a parser provided, for instance the Tailwind CSS parser
[`tailwindParser`](/reference/flex/tailwind-parser).
...
node tree. If a node (i.e. a [`<Box>`](/reference/flex/box) component) is added or removed
