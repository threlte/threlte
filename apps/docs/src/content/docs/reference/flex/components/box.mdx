...
of [`<Flex>`](/reference/flex/flex) or as a child of another `<Box>` to
...
- Using the [`useDimensions`](/reference/flex/use-dimensions) hook in a child component to `<Box>`:
...
To trigger a [layout reflow](/reference/flex/flex#layout-reflow), you can use the `reflow` slot prop:
