...
<Example path="flex/intro" showFile="Scene.svelte" />
...
Use the component [`<Flex>`](/reference/flex/flex) to create a flexbox
container. Since there's no viewport to fill, you must specify the size of the
container. Add flex items with the component [`<Box>`](/reference/flex/box).
...
The component [`<Align>`](/reference/extras/align) can be used to align the resulting flex container.
...
[`createClassParser`](/reference/flex/create-class-parser) which accepts a
...
class names](/reference/flex/tailwind-parser).
