...
[`useTask`](/reference/core/use-task). If adding a task to render the scene
to Threlte's
...
The hook [`useTask`](/reference/core/use-task) replaces `useFrame`. It has
...
[`useThrelte`](/reference/core/use-threlte).
...
The hook [`useTask`](/reference/core/use-task) also replaces `useRender`.
...
configuration in `svelte.config.js`. You can now use the [component `<T>`](/reference/core/t) directly.
...
The hook [`useLoader`](/reference/core/use-loader) now returns a custom Svelte store called
...
`EffectComposer` have been removed. Threlte 6 now provides a hook called [`useRender`](/reference/core/use-task#custom-render-pipeline) which
...
[`useFrame`](/reference/core/use-task) where the context is available as the first argument
to the callback:
...
The full type definition is [currently listed here](/reference/core/use-threlte).
