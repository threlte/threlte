...
Plugins allow you to extend Threlte's [`<T>`](/reference/core/t)
...
`@threlte/extras`](/reference/extras/interactivity) is an example of what a
...
[`<T>`'s `oncreate`](/reference/core/t#create-event) shares some
...
This plugin outlines the basic setup of the extras [bvh plugin](/reference/extras/bvh).
