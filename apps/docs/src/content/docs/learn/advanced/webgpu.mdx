...
1. Extend `<T>` with all the definitions from `three/webgpu` by using the [`extend`](/reference/core/t#extending-the-default-component-catalogue)
...
2. Use explicit imports for the objects, functions, and other classes that you use from `three/webgpu`.
   You can then use `<T>`'s [`is`](/reference/core/t#property-is) prop with those imports from `three/webgpu`.
...
3. Same as option #2 but using [`extend`](/reference/core/t#extending-the-default-component-catalogue) with the imports so that you can have `<T.MeshPhysicalNodeMaterial />` etc...
