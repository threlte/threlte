...
As a first step we're creating a new Svelte file called `App.svelte` where we are importing the [`<Canvas>` component](/reference/core/components/canvas).
...
In `Scene.svelte`, we're importing the [`<T>` component](/reference/core/components/t) which is
...
[plugin](/learn/advanced/plugins) [`interactivity`](/reference/extras/interaction/interactivity) from
