...
As a first step we create a new Svelte file called `App.svelte` where we import the [`<Canvas>` component](/reference/core/canvas).
...
In `Scene.svelte`, we import the [`<T>` component](/reference/core/t), which is
