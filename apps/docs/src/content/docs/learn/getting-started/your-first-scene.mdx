...
:::tip
Before rushing into Threlte, familarize yourself with svelte and threejs. Svelte's [Tutorial](https://svelte.dev/tutorial) is highly recommended as an introduction or revision. Threejs covers a lot of topics and their [fundamentals documentation](https://threejs.org/manual/#en/fundamentals) is a recommended resource for when you get stuck on a concept.
:::

## Structuring Your App

As a first step we're creating a new Svelte file called `App.svelte` where we are importing the [`<Canvas>` component](/reference/core/canvas).
...
will run. For improving access to this runtime context, it's <a href="/learn/basics/app-structure">best practice</a>
...
In `Scene.svelte`, we're importing the [`<T>` component](/reference/core/t) which is
...
<Example path="first-scene/step-1" hideCode />
...
<Example path="first-scene/step-2" hideCode />
...
<Example path="first-scene/step-3" hideCode />
...
Threlte supports listening to certain <a href="/reference/core/t#events">events on `<T/>` components</a>. Here, we use the `create`
...
[plugin](/learn/advanced/plugins) [`interactivity`](/reference/extras/interactivity) from
...
<Example path="first-scene/step-4" hideCode />
...
Let's add some motion to our cube. We will use Threlte's [`useTask`](/reference/core/use-task) hook to tap
...
<Example path="first-scene/step-5" hideCode />
...
<Example path="first-scene/step-6" hideCode />
...
<Example path="first-scene/step-7" hideCode />
