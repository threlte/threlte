...
`<T.Mesh onclick={onClick}>`. Threlte supports wheel, click and pointer events through the plugin
...
:::tip
[`CurrentWritable`](/reference/core/utilities#currentwritable) is a custom Threlte store.
