...
There are a number of different asset types you might like to use in your
Threlte application. Three.js has a [ton of different
loaders](https://threejs.org/docs/index.html?q=loader) that Threlte integrates
well with. You're recommended to use the
[useLoader](/reference/core/use-loader) hook which will cache any asset it
loads and wrap the asset in an
[asyncWritable](/reference/core/utilities#asyncwritable) for convenience.
Caching assets will reduce network requests, bandwidth, and memory usage, which
will improve the performance of your application.

You are highly recommended to use SvelteKit's [asset
function](https://svelte.dev/docs/kit/$app-paths#asset) so that you can catch common asset loading
problems. There are other equivalent vite plugins which will help you in the same way.
...
The return value from [useLoader](/reference/core/use-loader) is an
