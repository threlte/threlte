...
The [`<Canvas>`](/reference/core/components/canvas) component provides all basic contexts in a Threlte application.
...
will not**. The problem is that the `useTask` hook is called _outside_ of the
