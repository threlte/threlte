...
function from the [`useThrelte`](/reference/core/use-threlte) hook.
...
The [`useTask`](/reference/core/use-task) hook is by default configured to
...
[`useThrelte`](/reference/core/use-threlte) hook and set the `autoInvalidate`
option to `false`:
...
[`<Canvas>`](/reference/core/canvas) component:
...
[`useThrelte`](/reference/core/use-threlte) hook:
...
you may want to render the scene yourself, for example when using [postprocessing](/examples/postprocessing/outlines).

1. Set `autoRender` to `false` on the [`<Canvas>`](/reference/core/canvas)
...
- Use `shouldRender` from the hook
  [`useThrelte`](/reference/core/use-threlte). This function will evaluate
