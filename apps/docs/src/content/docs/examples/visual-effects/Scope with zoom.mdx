...
This example shows how to utilize the [`useFBO`](/reference/extras/use-fbo) hook to create a sniper scope zoom effect, complete
...
from [`useFBO`](/reference/extras/use-fbo) hook is used in the scope where a
...
The [useFBO](/reference/extras/use-fbo) hook is used to prepare a render target for the scope's view texture.
