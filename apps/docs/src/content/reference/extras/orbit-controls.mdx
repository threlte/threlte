...
the renderer is rendering to as the DOM element to listen to. It will also invalidate the frame loop
when changes occur. If the `autoRotate` or `enableDamping` properties are set to `true`, the frame loop will run continuously.
