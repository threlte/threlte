...
The component `<InstancedMeshes>` takes existing `THREE.Mesh` instances and creates a `THREE.InstancedMesh` per `THREE.Mesh`. This is especially useful if you want to instanciate a lot of meshes that have been loaded with hooks like [`useGltf`](/docs/reference/extras/use-gltf).
...
Load a gltf file with the [`useGltf`](/docs/reference/extras/use-gltf) hook and pass the result to the `<InstancedMeshes>` component. The [slot prop](https://svelte.dev/docs#template-syntax-slot) `components` can be used to instanciate a mesh multiple times.
