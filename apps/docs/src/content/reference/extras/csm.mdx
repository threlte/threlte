...
To enable CSM, it's essential to set `renderer.shadowMap.enabled` to `true`,
which can be done via renderer.shadowMap.enabled = true.

Then, wrap all of the objects that you wish to use CSM on (typically your entire scene) with the `<CSM>` component.
Making a CSM wrapper, as demonstrated in our example, is recommended, particularly for more advanced configurations.
Making a wrapper nested inside of `<Canvas>`, enables the use of Threlte hooks, such as `useThrelte`.
...
The `<CSM>` component exposes an optional `params` prop. Parameters are not reactive
and are only updated when CSM is enabled.

### Updating light direction

To modify the light's direction, utilize the `lightDirection` param,
which should be of the type `THREE.Vector3`. After setting this parameter, any subsequent
adjustments should be made directly to this vector. Light direction vector hast o be normalized.
...
The `<CSM>` component offers a configure callback, which is triggered when CSM is activated.
