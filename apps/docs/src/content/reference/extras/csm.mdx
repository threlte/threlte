...
For CSM to work, shadow maps must be enabled. Threlte does this by default. Be sure to _not_ disable shadowmaps by [`<Canvas shadows={false}>`](/docs/reference/core/canvas#component-signature).
...
To modify the light's direction, utilize the `lightDirection` parameter,
which should be of the type `THREE.Vector3`. After setting this parameter, any subsequent
adjustments should be made directly to this vector. The light direction vector hast to be normalized.
...
The `<CSM>` component offers a configuration callback, which is triggered when CSM is activated.
