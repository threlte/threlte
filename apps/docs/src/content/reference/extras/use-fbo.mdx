...
`useFBO` accepts an options getter. Reads inside the getter are tracked through runes, so changes to `size` and `depth` are applied to the render target reactively. Render-target construction options (`format`, `type`, `samples`, …) are read once at hook initialization.
