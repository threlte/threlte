...
instance to track a moving target — the missing gameplay layer on top of an
orbit camera. The component handles orbit, zoom, touch input, collision
...
<script lang="ts">
  ...
  $effect(() => {
    if (character) follow.follow(character)
  })
</script>
...
Call `follow.follow(target)` to start tracking. The target can be any of:

- An `Object3D` directly
- A ref-like object with a `current` property
- A getter function that returns an `Object3D | null | undefined`

`follow.follow` is also usable as an `oncreate` handler on a `<T>` component:

```svelte
<T.Group oncreate={follow.follow}>
  <!-- character mesh -->
</T.Group>
```

Call `follow.unfollow()` to stop.
...
| `colliderMeshes` (collision)             | target via `follow()`    |
| `pointerLock` (input mode)               |                          |
