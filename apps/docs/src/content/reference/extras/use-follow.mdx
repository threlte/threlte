...
- `task` — the main update, in `mainStage`. Sets the controls target (with
  `followSmoothTime` smoothing baked in) and azimuth. Schedule movement
  **before** this so the camera sees the final target position for the frame.
- `postTask` — runs after `<CameraControls>`' own update, in a dedicated
  stage between `mainStage` and `renderStage`. Reads the final camera pose to
  update reactive state. Schedule camera-decorating work (e.g. a camera shake)
  **after** this.
