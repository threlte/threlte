...
`useFollow` runs its update in two named tasks:

- `task` — the main update, in `mainStage`. Sets the controls target and
  azimuth from the current target position. Schedule movement **before** this
  so the camera sees the final target position for the frame.
- `smoothTask` — runs after `<CameraControls>`' own update, in a dedicated
  stage between `mainStage` and `renderStage`. Applies `followSmoothTime`
  lag and rebuilds the camera's `lookAt`. Schedule camera-decorating work
  (e.g. a camera shake) **after** this so it isn't overwritten.
