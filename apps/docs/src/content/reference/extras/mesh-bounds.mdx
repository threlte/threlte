...
You may use [Threlte's plugin system](/docs/learn/advanced/plugins) to
automatically apply the `meshBounds` raycast function to all or selected meshes
in your scene.
...
<script>
  ...
  injectPlugin('mesh-bounds-plugin', ({ ref, props }) => {
    if ('raycast' in ref && typeof ref.raycast === 'function') {
...
<script>
  ...
  injectPlugin('mesh-bounds-plugin', ({ ref, props }) => {
    if ('raycast' in ref && typeof ref.raycast === 'function' && !!props.raycastMeshBounds) {
      ref.raycast = meshBounds
    }
    return {
