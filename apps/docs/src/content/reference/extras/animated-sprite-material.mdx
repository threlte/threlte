...
`<AnimatedSpriteMaterial>` can be attached to not only a `<T.Sprite>`, but also a `<T.Mesh>`.

<Example path="extras/animated-sprite-material-mesh" />

In this case, the sprite will use a `MeshBasicMaterial` by default. It will also create a custom depth material to support shadows.
