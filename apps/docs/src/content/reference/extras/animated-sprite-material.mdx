...
In the case of a `Mesh` parent the sprite will use a `MeshBasicMaterial` by default, instead of a `SpriteMaterial` when attached to a `Sprite`. A custom depth material will be attached when parented to a mesh to support shadows.
