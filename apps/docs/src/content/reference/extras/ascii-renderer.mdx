...
Sometimes you only need to render a single frame. With `options.color` set to false, it isn't much of a problem to have a render task that runs indefinitely. However, setting `options.color` to true and only needing to render one frame or on demand can be too slow. For this reason `<AsciiRenderer>` accepts an `autoRender` prop. If `autoRender` is `true`, the render task will start and run until `autoRender` is set to `false`.

<Example path="extras/ascii-renderer/auto-render" />

In the example above, `autoRender` is set to `true` and rendering occurs as it normally would. `<AsciiRenderer>` is paused when `autoRender` is unchecked. This means that the renderer doesn't need to run expensive frames if you only want one frame to be captured.
