...
sourcePath: 'packages/extras/src/lib/components/LinearGradientTexture/LinearGradientTexture.svelte'
...
componentSignature:
  {
    ...
    props:
      [
        {
          default: '1024',
          description: "width of the gradient's canvas",
          name: 'width',
          required: false,
          type: 'number'
        },
        {
          default: '1024',
          description: "height of the gradient's canvas",
          name: 'height',
          required: false,
          type: 'number'
        },
...
A reactive gradient texture which attaches to the "map" property of its parent.

<Example path="extras/linear-gradient-texture" />

## Gradient Stops

`<LinearGradientTexture>` accepts a `stop` prop. Stops are defined by two things; an offset and a color. Gradient stops are identical to how you would use them with a 2D context, notably the offset of a stop should be between 0 and 1 inclusive. Colors can be any valid color representation in ThreeJS. Here are a couple of examples of valid stops.
