...
`<GLTF />` is loads a single model. Changing the property `url` loads new 3D content. The new content will be swapped as soon as loading is finished.

To have multiple copies of the same model or to customise parts of your model (i.e. casting shadows), use the [gltf package](docs/reference/gltf/getting-started), or create your own component using the [`useGltf`](/docs/reference/extras/use-gltf) hook.
