...
To use KTX2 compressed textures, import the `useKtx` hook or provide an instance of a `KTX2Loader`.

```svelte
<script>
  import { useKtx } from '@threlte/extras'

  const ktx = useKtx('path/to/transcoder') // Creates a cached instance of KTX2Loader
</script>
