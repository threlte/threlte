...
For cases where you want full control over when the cube camera updates, there is an `update` function available as a component export and through the `children` snippet. Be sure to set `frames` to `0` if you use this feature.
