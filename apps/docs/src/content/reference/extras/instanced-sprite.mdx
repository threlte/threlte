...
[sprites](https://en.wikipedia.org/wiki/Sprite_(computer_graphics)) in your scene. You can instance
sprites using the `useInstancedSprite` hook, or with a `<Instance>` component available through a slot prop.

The component lets you configure your sprites in an
open data format, which you can build yourself or a convenience `buildSpritesheet` utility. You can find an example of a [complex scene here](/docs/examples/animation/complex-sprite-scene).
	...
	signature={{
      props:
        [
            ...
            type: 'boolean',
            required: false,
            description: 'Toggles looping of the animation on/off. If off, the last played frame is displayed when the animation completes.'
          },
					{
            name: 'flipX',
            type: 'boolean',
