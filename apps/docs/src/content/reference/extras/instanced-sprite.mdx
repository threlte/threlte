...
<Tip type="experimental">This is an early version - features and API might change significantly over time. Please report any issues you encounter on Discord or Github.</Tip>
...
- **`type`** - choose whether you’re providing how many rows and columns are in an image `"rowColumn"` or giving the size of a single
