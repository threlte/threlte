...
componentSignature:
  {
    extends: { type: 'Group', url: 'https://threejs.org/docs/#api/en/objects/Group' },
		exports: [
      {
				description: 'restarts the internal update task',
        name: 'restart',
        type: '() => void'
      },
      {
...
If you want full control over updates, there is an `update` function that is available as a component export and through the `children` snippet.

<Tip type="warning">
  Be sure to set `frames` to `0` if you are using the `update` function. This will prevent the
  internal update task from automatically starting.
</Tip>
...
```svelte title="Through-The-Children-Snippet"
<script>
  let cubeCameraComponent = $state()
