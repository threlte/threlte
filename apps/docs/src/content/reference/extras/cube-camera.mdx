...
For moving objects, let `frames` default to `Infinity`. If you have a static scene, set `frames` equal to the number of `<CubeCameras>` in the scene. This will allow each one to render and then be picked up in the each other's reflection.
