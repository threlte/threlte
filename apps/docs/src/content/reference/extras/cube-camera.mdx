...
The `onrenderstop` callback runs anytime the render task stops running. It is called on restarts and when the internal counter goes over the `frames` limit. This means that if `frames` is set to `Infinity`, `onrenderstop` is never called.
