...
componentSignature:
  {
    props:
      [
        { description: 'the url to a texture to load and use', name: 'url', type: 'string' },
        {
        ...
        {
          default: 'true',
          description: 'whether to set `scene.environment` to the loaded environment texture',
          name: 'isEnvironment',
          type: 'boolean'
        },
        {
...
3. `scene.environment` and/or `scene.background` are still set to the environment texture based on `isEnvironment` and `isBackground`. This is done so that the environment map can still affect materials used in the scene.
