...
`<Environment>` has a `resource` prop that can either be a `Texture`, `DataTexture` or `string`. If `resource` is not a string, the provided resource will be used when setting the scene's environment. If `resource` is a string, `<Environment>` will load the texture using a corresponding `Loader`.

The example below passes a loaded texture to `<Environment>` instead of loading one.
