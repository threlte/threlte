...
The default `loadOptions` sets `texture.mapping` to `Three.EquirectangularReflectionMapping`. `loadOptions` is **not** used if `resource` is a `Texture` or `DataTexture`.
