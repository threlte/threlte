...
The `loadOptions` property can be used to modify a loaded texture before it is assigned to `scene.environment`. `loadOptions` is used inside the loader's `load` function and accessed through a callback. This means that it is only read when **after** a texture is loaded. By default, `loadOptions` will set the loaded texture's `mapping` property to `THREE.EquirectangularReflectionMapping`.
...
This is not recommended. The preferred way reuse a texture is to load the texture using [useTexture](/docs/reference/extras/use-texture) modify it, and then pass that into `<Environment>` and to any other components or as props as explained in the next section

### Bindable `texture` Prop
...
Be aware that `<Environment>` will set the `texture` bindable prop **after** it has loaded a texture. This means that if you provide both `url` and `texture` properties, the texture at `url` will eventually be assigned to `texture`.
...
<script>
  ...
  let index = $state(0)
  const resource = $derived(urls[index])
