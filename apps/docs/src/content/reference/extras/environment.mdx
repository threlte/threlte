...
As previously mentioned, if `resource` is a file path, it should at least be something that can be loaded by `Three.TextureLoader`.

## Loader Options

`<Environment>` uses [useLoader](/docs/reference/core/use-loader) internally to create a loader for loading environment texture files. `loaderOptions` props are only used when loading a texture. `loaderOptions` is ignored if `resource` is a `Texture` instance.

## Modifying The Texture

`<Environment>` exposes the loaded texture through its `texture` property if you need to modify the texture. Note that `<Environment>` will set `texture.mapping` to `Three.EquirectangularReflectionMapping` the first time the texture is loaded. Likewise, the scene's environment is only set after a texture is loaded. What this means is even if you update the texture, the environment will not be updated until a new texture has to be loaded or retrieved from the cache.

The example below demonstrates modifying the texture. Toggling the checkbox does not cause the texture to flip but if you toggle the checkbox _and then_ swap the url, the displayed texture will be modified.
...
If `resource` is an instance of a `Texture` or `DataTexture`, the suspense is immediately resolved.
