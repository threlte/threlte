...
As previously mentioned, if `resource` is a flepath, it should at least be something that can be loaded by `Three.TextureLoader`.
...
`loadOptions` is only accessed **before** the texture is assigned to `scene.environment`. This means that if you update `loadOptions`, you will not see its effects until the loader has to load a new texture.
...
1. `skybox` is only set to a `GroundedSkybox` instance if `ground` !== `false` and **after** the texture is available. It is set to `undefined` when the `<Environmen>` unmounts.
