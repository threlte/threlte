...
To use Threlte transitions, you need to inject the [plugin](http://localhost:3000/docs/reference/core/plugins) first via invoking `transitions()`. All child `<T>` components will then accept transition properties.
