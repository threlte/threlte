...
When `<BakeShadows>` is used within a `<Suspense>`, shadows will be frozen after the boundary is no longer in a suspended state.

<Example path="extras/bake-shadows/spaceships" />

The example above is a good use-case for `<BakeShadows>` because all objects in the scene are stationary and the shadows will never change.
