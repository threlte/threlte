...
A hook that can be used to give a scene an asciiEffect. It is a thin wrapper around the `AsciiEffect` [addon](https://threejs.org/examples/?q=as#webgl_effects_ascii).

<Example path="extras/use-ascii-effect" />

## Usage

Start by disabling the [autorendering](/docs/reference/core/canvas#component-signature) on your canvas i.e. `<Canvas autoRender={false}>`, then call this hooks render function within your `useTask`.
