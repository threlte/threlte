...
<Tip type="tip">
  Be aware that `useStage` never removes a stage as that's usually not needed. If a stage
	is created *once* in your Threlte app, it's available throughout the app's lifetime.
</Tip>
...
`mainStage` (making it the first stage) and another stage that runs after
Threlte's `renderStage` (making it the last stage). We then call `perf.begin()`
in a task of the first stage and `perf.end()` in a task of the second stage.
