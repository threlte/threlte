...
Watch a single or multiple reactive values and call a callback when they change.
This is the runes-equivalent to `watch`.
...
observe(
  () => [count],
  ([count]) => {
    // This callback is only called when `count` changes
