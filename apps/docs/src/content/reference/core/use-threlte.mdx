...
console.log(renderer, camera.current)
```

… or read render properties:

```ts
const { toneMapping } = useThrelte()
console.log(toneMapping.current)
```
...
If the [renderMode is set to `'manual'`](/docs/reference/core/canvas) you must manually trigger a re-render using `invalidate()`:
