...
By default, `<T/>` doesn't have any click, pointer or wheel events; however, interaction events
(using raycasting) can be enabled using the [`interactivity`](/docs/reference/extras/interactivity)
plugin.
