...
Three.js objects are **class instances**. When Instantiating these objects, they
can receive one-time constructor arguments (`new THREE.SphereGeometry(1, 32)`).
In Threlte, constructor arguments are always passed as an array via the property
...
<Tip type="tip"> Be aware that in this case `<T>` can't provide type-safety on
  properties and events. If you need type-safety, you can use the `is` property
  instead: `<T is={CustomMesh} />`. This way, Threlte can infer the type of the
  object and provide type-safety on properties and events. </Tip>
