...
- `attach` can also be a function which is called on mounting with the parent and the value inferred from the property `is`. It can return a function which is called `onDestroy`:
