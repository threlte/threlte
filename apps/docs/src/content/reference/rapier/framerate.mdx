...
_advance_ the physics simulation. This implies that realistically no two
executions of the same physics simulation will yield the same result. In short:
it's _not deterministic_.
...
Sometimes however, you want a physics simulation to always yield the same
result, e.g. to be _deterministic_. In this case, you can set the `framerate`
