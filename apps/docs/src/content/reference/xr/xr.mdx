...
componentSignature:
  {
    props:
      [
        ...
        {
          name: 'handFactory',
          type: 'XRHandModelFactory',
          description: 'Allows a custom XRHandModelFactory to be used during hand setup'
        },
        {
          name: 'controllerFactory',
          type: 'XRControllerModelFactory',
          description: 'Allows a custom XRControllerModelFactory to be used during controller setup'
        },
        {
          name: 'referenceSpace',
          type: 'string',
        ...
        {
          name: 'enterGrantedSession',
          type: 'boolean | XRSessionMode[]',
          default: 'true',
          description: 'Auto-enter a session when the OS grants one without an explicit request (e.g. when the user puts on a headset). Pass `false` to disable, or an array of modes (e.g. `["immersive-vr"]`) to restrict which modes are eligible.'
        },
        {
          name: 'offerSession',
          type: 'boolean | XRSessionMode',
          default: 'true',
          description: 'Pre-offer a session via `navigator.xr.offerSession` so the browser can surface its own entry UI (e.g. visionOS). When `true`, offers AR if supported, otherwise VR. Pass a specific mode to restrict, or `false` to disable.'
        }
      ],
    events:
      [
        ...
        {
          name: 'frameratechange',
          payload: 'THREE.Event & { target: XRSession }',
          description: 'Fired when the session frame rate changes.'
        }
      ]
  }
---
