...
<XR>
  <Text text='I have entered another realm!' />
</XR>
```

<Tip type="warning">
  The `<XR>` component will set the [`<Canvas>`](/docs/reference/core/canvas) property `frameloop="always"` when the user enters an XR session, due to being incompatible with `demand` or `never`. It will set the original value once the session has ended.
</Tip>

Any children of the `<XR>` component will not mount until the user enters an immersive session. 

This is useful for adding controllers, hands, or entire scenes that should only start when the user has begun their session.