...
returns `NodeProps`. Let's assume, you want to create a parser that supports the
...
```ts
import type { createClassParser } from '@threlte/flex'

const classParser = createClassParser((string, props) => {
  // First we need to split the string into an array of class names
  const classNames = string.split(' ')
  // We then loop over each class name …
  for (const className of classNames) {
    // … and check if it matches one of the predefined class names
    switch (className) {
      case 'container':
        // If it matches, we assign the corresponding props.
        // We don't need to set display: flex, since that is the default.
