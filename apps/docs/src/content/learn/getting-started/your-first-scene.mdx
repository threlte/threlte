...
<Tip type="info">
  You should be familiar with both Svelte and Three.js before use Threlte. If you are new to Svelte,
  consult its [Tutorial](https://svelte.dev/tutorial) for a quick introduction. As for Three.js, its
  [official documentation](https://threejs.org) has many resources for getting started.
</Tip>
...
We now want to view our cube with some perspective. To easily manipulate a camera, let's add one to our scene:
