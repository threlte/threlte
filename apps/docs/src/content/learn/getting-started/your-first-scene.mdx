...
Let's add some motion to our cube. We will use Threlte's [`useTask`](/docs/reference/core/hooks#useTask) hook to tap
...
`useTask` registers a callback that will be invoked on every frame. The function receives two arguments: the
current state of the Threlte application (the same context available via the
[`useThrelte`](/docs/reference/core/hooks#useTask)) and the time delta since the last frame. We use
