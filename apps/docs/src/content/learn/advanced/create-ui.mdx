...
extensively across Threlte. Anytime you make a
[useThrelte](/docs/reference/core/use-threlte) call, the context API is being
used.
...
[class](https://svelte.dev/docs/svelte/$state#Classes) to create reactive
context state for your UI.
...
needed. This is the global state option.
...
Remember that events only bubble up to ancestors. If you've created an html
element that covers your canvas, for example:
...
Your `<Scene>` will not receive any mouse events because they can only bubble
up. In this case you need to add [pointer-events:
