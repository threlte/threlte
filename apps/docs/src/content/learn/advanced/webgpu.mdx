...
If the `<T>` component doesn't have the extra WebGPU class definitions, it will
error when trying to do things like `<T.MeshPhysicalNodeMaterial />`. To sort this out
use the [`extend`](/docs/reference/core/t#extending-the-default-component-catalogue) function.
...
Previously, libraries such as [three-custom-shader-material](https://www.npmjs.com/package/three-custom-shader-material)
use a find and replace solution. This works but threejs has identified
that it's not an ideal solution and recommends using the Threejs Shading Language,
otherwise known as [TSL](https://github.com/mrdoob/three.js/wiki/Three.js-Shading-Language).

The example below is an adaptation of [this](https://threejs.org/examples/?q=tsl#webgpu_tsl_angular_slicing)
threejs example. There are many more [TSL examples](https://threejs.org/examples/?q=tsl#webgpu_tsl_angular_slicing)
within three.js that you can use or adapt for your project. This is not a beginner's guide to shaders.
...
needs its `side` set to `THREE.DoubleSide` otherwise as these fragments are facing away
from the camera, threejs would cull them out.

Any fragment that is discarded in the shadowNode will not cast shadows.

### Updating Uniforms

If your node depends on uniforms, these can be declared in the `script` tag of
...
The material above demonstrates two ways to update uniforms. The `uTime` uniform
is updated in a `useTask` callback. `uIntensity` is modified in a `$effect`. Either way the uniform updates are picked up in the material.

TSL has an [`oscSine`](https://github.com/mrdoob/three.js/wiki/Three.js-Shading-Language#oscillator) function that oscillates on time that could also be used in the example above.
