...
By default `<T>` doesn't know about the new classes and will
error on things like `<T.MeshPhysicalNodeMaterial />`. You have two options.

1. Give `<T>` all the new classes from `three/webgpu` with the [`extend`](/docs/reference/core/t#extending-the-default-component-catalogue)
   function. Adding all the classes will increase the bundle size of your application
   but is easy and works.
...
2. Be explicit and import only the classes you need from `three/webgpu`. Then either:

**a)** use `<T>`'s [`is`](/docs/reference/core/t#property-is) prop with the webgpu
classes you initialize within your `<script></script>` tags
...
or **b)** [`extend`](/docs/reference/core/t#extending-the-default-component-catalogue)
`<T>` with those classes.
...
Bundle size of your application will remain minimal but you'll have to keep things updated as you go.
