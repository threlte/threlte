...
`@threlte/extras` ships a matching `webgpu` export. Import from it instead of
...
Most of the package is renderer-agnostic and works as-is, a few components have
been rewritten in TSL, and the rest throw until they are migrated. See
[`@threlte/extras` WebGPU](/docs/reference/extras/webgpu) for the full breakdown.
