...
Plugins allow you to extend Threlte's [`<T>`](/docs/reference/core/t) component. They can be used to add props, event handlers, custom logic and customize the component instance. They are similar to [svelte actions](https://svelte.dev/docs/svelte/use) but differ in that the object referenced by a `<T>` component may be recreated when the plugin's `args` change.

`T`'s [`oncreate`](/docs/reference/core/t#create-event) event prop is also similar to svelte actions. They are similar tools but the main difference is that plugins are hierarchical. Injecting a plugin will run the plugin for the `<T>` component and any of its children. `oncreate` will only rerun if something causes the underlying instance of the `<T>` component to be recreated such as when `args` updates.

Note that plugins can be overridden in child components.

The [interactivity plugin in `@threlte/extras`](/docs/reference/extras/interactivity) is an example of what a plugins can do and there are a couple of other examples below.

## Injecting a Plugin

Plugins are _injected_ to a plugin context and are **accessible to all child `<T>`
components**.
