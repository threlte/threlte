...
Threlte's [`oncreate`](/docs/reference/core/t#create-event) event is also similar to svelte `actions`. They are similar tools but a big difference is that plugins are hierarchical. Injecting a plugin will add that code to every child `<T>` component instance whereas a custom `oncreate` event would need to be added to each `<T>` component individually. Plugins can be overridden in child components.
