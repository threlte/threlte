...
Threlte's [`<T>`](/docs/reference/core/t) component functionality. This means
you can not only add arbitrary props and event handlers to your `<T>` components
but also override the default behavior of the `<T>` component entirely.

Plugins add code to every `<T>` component instance. That code acts as if it were
part of the `<T>` component itself. You have full access to props, event
