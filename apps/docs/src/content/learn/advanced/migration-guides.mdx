...
When passing `false` as the attach prop, the component will not be attached to
...
When passing an object3D as the attach prop, the component will be attached to
the provided object, essentially acting as a portal.
...
You can find a more in-depth WebGPU example [here](/docs/examples/renderers/webgpurenderer).
