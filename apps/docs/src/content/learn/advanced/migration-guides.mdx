...
Threlte 8 marks a large milestone including a number of performance improvements, refactors,
breaking changes, API simplifications, and removed features.
Svelte 5 support is added and **support for Svelte 4 is removed**.
...
For example the following code from Threlte 7 needs to be migrated from this:
...
The `userSize` property on the `<Canvas>` component and returned from the `useThrelte` hook has been removed.
