...
## Migrating from `useFrame` and `useRender` to the Task Scheduling System

Threlte 6 introduces a new task scheduling system that allows you to easily
...
ever executes its tasks when a re-render is needed.
