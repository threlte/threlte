...
parts (i.e. *components*), we run into the issue that we need to run multiple
callbacks on every frame that may be dependent on each other. For instance, we
...
In this section, we will learn how to use the easy-to-use tools
Threlte provides to create and orchestrate stages and tasks.

<img class="w-full max-md:hidden" src="/images/docs/learn/frame-handling.svg" />
<img class="w-auto h-[85vh] mx-auto md:hidden" src="/images/docs/learn/frame-handling_portrait.svg" />
...
default, the created task is added to [Threlte's `mainStage`](#default-stages)
