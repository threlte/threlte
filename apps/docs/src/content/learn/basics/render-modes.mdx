...
battery usage of your Threlte app. Ideally, you only want to render the scene
...
Threlte is able to automatically invalidate the current frame by observing the
mounting and unmounting of components and component props. This means that when
you e.g. change the position of a `<T.Mesh>` via component properties, Threlte
...
<Tip type="tip">
	The renderMode property can be changed at any time, but it will only
