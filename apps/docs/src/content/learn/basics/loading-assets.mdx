...
There are a number of different assets you might like to use in your Threlte application. Below are sections to guide you on how to get started with them. Three.js has a [ton of different loaders](https://threejs.org/docs/index.html?q=loader) and Threlte can use all them. You're recommended to use the [useLoader](/docs/reference/core/use-loader) hook to cache assets and for convenience in asynchronous setups. This will help reduces network requests, bandwidth, memory usage and improve the performance of your application.
