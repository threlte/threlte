...
There are a number of different assets you might like to use in your Threlte application. Below are sections to learn about how to get started with them. Three.js has a [ton of different loaders](https://threejs.org/docs/index.html?q=loader) and Threlte can use all them. You're recommended to use the [useLoader](/docs/reference/core/use-loader) hook to cache assets and for convenience in asynchronous setups. This will help reduces network requests, bandwidth, memory usage and improve the performance of your application.

<Tip type="info">
  It is assumed you placed your assets in your public folder or in a place in your application where
...
If you wanted to load a texture for the `map` and `normalMap` channels, you could do that like so:
...
You can then spread the textures on a material via Svelte's spread syntax:
...
<Tip type="tip">
  The promise only resolves, and the svelte store populated, once all textures have loaded.
</Tip>
...
To help orchestrate loading multiple assets before displaying something, you can use the [suspense](/docs/reference/extras/suspense) component and hooks.
