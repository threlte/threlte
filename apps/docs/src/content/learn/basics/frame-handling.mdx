...
– from rotating an object continously around an axis to reacting to user input.
Web-based apps rely on the browsers `requestAnimationFrame` that runs a callback
when a new frame is rendered. When encapsulating logic into smaller parts (i.e.
*components*), we run into the problem that we need to run multiple callbacks on
every frame. We also need to make sure that these callbacks are executed in a
specific order.

In Threlte, these functions are called **tasks** and may or may not follow a
specific order. If an order is specified, the respective tasks have a
**dependency** to another task. Tasks are grouped into **stages** which follow
the same logic and may or may not have dependencies to other stages to be
executed in a specific order. A Threlte app is managed by a single
**scheduler**. In this section, we will learn how to use the easy-to-use tools Threlte
provides to create and orchestrate stages and tasks.
...
<Tip type="info">
  ...
  used internally by Threlte. However, you can use it to create stages and run tasks
