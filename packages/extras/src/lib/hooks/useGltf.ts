...
/**
 * The default `Graph` types `nodes` and `materials` as `Record<string, any>` on
 * purpose. When a caller does not supply an explicit `Graph`, they should be
 * able to reach into the scene graph — e.g. `gltf.nodes.SomeMesh.geometry` or a
 * material's shader properties — without casting each access. Callers who want
 * strict typing pass their own `Graph` (for instance the one generated by
 * `@threlte/gltf`), which overrides these permissive defaults. The narrowly
 * scoped disable below keeps this intentional `any` from being flagged.
 */
